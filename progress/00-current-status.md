...
Use the stabilized smoke loop plus the new SVG snapshot path to harden policy injection, icon semantics, and additional runtime node-family coverage.

## Immediate Next Steps

1. Wire component policy rule injection into the live plugin runtime so preserved instances are not always the default.
2. Improve preserved-instance snapshot fidelity where current exports still lack internal semantics, especially app bars, carousels, and icon swaps.
3. Re-run the full automated suite in an environment that permits localhost listeners so the companion HTTP tests can execute.
